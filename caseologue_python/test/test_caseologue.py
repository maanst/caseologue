...
class IncorrectNumberError(Exception):
    ...
    def __init__(self, nb_expected_error, nb_detected_errors,
                 message_start="Incorrect number of errors caught by the test: ", message_end=" instead of "):
...
def test_caseologue(name, nb_expected_error):
    os.environ[
        'EDAM_PATH'] = dir_path + "/data/" + name + "_test_data.owl"  # get the test data from the name using the full path based on the directory path
...
if __name__ == "__main__":
    ...
    # test_caseologue(name= "missing_deprecated_property", nb_expected_error=3) # New store = no errors found
    ...
    test_caseologue(name="format_property_missing", nb_expected_error=2)  # 3 errors originally, only 2 with new store
